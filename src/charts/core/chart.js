class G_FinancialChart {
  ...
  renderKLineChart() {
    ...
    // 计算坐标映射
    const xScale = (width - padding.left - padding.right) / (this.data.length - 1);
    const yScale = (height - padding.top - padding.bottom) / priceRange;
    
    // 绘制 K 线
    this.data.forEach((item, index) => {
      const x = padding.left + index * xScale;
      const wickTop = padding.top + (height - padding.top - padding.bottom) - (item.high - minPrice) * yScale;
      const wickBottom = padding.top + (height - padding.top - padding.bottom) - (item.low - minPrice) * yScale;
      const bodyTop = padding.top + (height - padding.top - padding.bottom) - (Math.max(item.open, item.close) - minPrice) * yScale;
      const bodyBottom = padding.top + (height - padding.top - padding.bottom) - (Math.min(item.open, item.close) - minPrice) * yScale;
  ...
  renderTimeChart() {
    ...
    this.data.forEach((item, index) => {
      const x = padding.left + index * xScale;
      const y = padding.top + (height - padding.top - padding.bottom) - (item.close - minPrice) * yScale;
