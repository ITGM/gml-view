...
class G_MACD extends G_Indicator {
  constructor(fastPeriod = 12, slowPeriod = 26, signalPeriod = 9, colors = {
    macd: '#FF6B6B',
    signal: '#4ECDC4',
    histogram: '#FFD166'
  }) {
  ...
  calculate(data) {
    ...
    for (let i = this.signalPeriod; i < macdLine.length; i++) {
      const currentSignal = (macdLine[i] - signalLine[i - this.signalPeriod]) * signalMultiplier + signalLine[i - this.signalPeriod];
  ...
  render(ctx, data, options) {
    ...
    macdData.macdLine.forEach((value, index) => {
      ...
      const y = padding.top + (height - padding.top - padding.bottom) - (value - minValue) * yScale;
    ...
    macdData.signalLine.forEach((value, index) => {
      const x = padding.left + (index + this.slowPeriod + this.signalPeriod - 2) * xScale;
      const y = padding.top + (height - padding.top - padding.bottom) - (value - minValue) * yScale;
    ...
    macdData.histogram.forEach((value, index) => {
      const x = padding.left + (index + this.slowPeriod + this.signalPeriod - 2) * xScale;
      const y = padding.top + (height - padding.top - padding.bottom) - (0 - minValue) * yScale;
